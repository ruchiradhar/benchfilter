...
def main() -> None:
    output_dir = PROJECT_ROOT / "results" / "core_analysis" / "dif_anchor"
    ...
    stab_mgsm = pd.read_csv(
        PROJECT_ROOT / "results" / "core_analysis" / "gsm_1pl" / "item_rank_stability.csv",
        index_col="item_id",
    )
    ...
    for lang in MGSM_FOCAL:
        ...
        r, items_df = run_dif(ref_mgsm, foc, anchor_mgsm)
        ...
        items_df.insert(0, "benchmark", "MGSM")
        items_df.insert(1, "domain", "-")
        items_df.insert(2, "focal", lang)
        item_rows.append(items_df)

    # ── MMLU ──────────────────────────────────────────────────────────────────
    print("\n=== MMLU ===")
    stab_mmlu = pd.read_csv(
        PROJECT_ROOT / "results" / "core_analysis" / "mmlu_1pl" / "item_rank_stability.csv",
    )
    ...
    for domain in MMLU_DOMAINS:
        ...
        for lang in MMLU_FOCAL:
            ...
            r, items_df = run_dif(ref_mmlu, foc, anchor_d)
            ...
            items_df.insert(0, "benchmark", "MMLU")
            items_df.insert(1, "domain", domain)
            items_df.insert(2, "focal", lang)
            item_rows.append(items_df)
    ...
    items_all = pd.concat(item_rows, ignore_index=True)
    items_all.to_csv(output_dir / "dif_anchor_items.csv", index=False)
    print(f"Saved dif_anchor_items.csv to {output_dir}")

    pp_all = (0.5 - expit(-items_all["delta_b"].abs())) * 100
    pp_dif = (0.5 - expit(-items_all.loc[items_all["dif_fdr05"], "delta_b"].abs())) * 100
    print(f"\nProbability-point shift at item difficulty (both benchmarks pooled, "
          f"n={len(items_all)} tested items): mean={pp_all.mean():.1f}pp, median={pp_all.median():.1f}pp")
    print(f"Probability-point shift among DIF-flagged items only (n={items_all['dif_fdr05'].sum()}): "
          f"mean={pp_dif.mean():.1f}pp, median={pp_dif.median():.1f}pp")
    for bench in ["MGSM", "MMLU"]:
        sub = items_all[items_all["benchmark"] == bench]
        pp_b = (0.5 - expit(-sub["delta_b"].abs())) * 100
        print(f"  {bench} all tested: mean={pp_b.mean():.1f}pp  "
              f"(n={len(sub)})")
