#!/usr/bin/env python3
"""Train 1PL IRT models from all files in data/irt and save results.

Examples:
    python scripts/irt/irt_onepl.py
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a 1PL IRT model for every jsonlines file in data/irt."
    )
    parser.add_argument(
        "--data_dir",
        type=Path,
...
def setup_logger(log_dir: Path) -> tuple[logging.Logger, Path]:
    ...
    log_path = log_dir / f"irt_onepl_{run_date}.log"
