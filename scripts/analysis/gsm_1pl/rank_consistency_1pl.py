...
Examples:
    ...
    python scripts/analysis/gsm_1pl/rank_consistency_1pl.py --results_dir results/1PL --output_dir results/analysis/gsm_1pl
"""
...
def plot_spearman_heatmap(corr_df: pd.DataFrame, w: float, langs: list[str], ax: plt.Axes) -> object:
    ...
    ax.set_title(f"MGSM 1PL — Item difficulty rank correlation\nKendall's W = {w:.3f}", fontsize=10)
    return im
...
def plot_item_stability(rank_std: pd.Series, ax: plt.Axes) -> None:
    ...
    ax.set_title("MGSM 1PL — Item rank stability (lower = more consistent)", fontsize=10)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output_dir", type=Path, default=Path("results/analysis/gsm_1pl"))
